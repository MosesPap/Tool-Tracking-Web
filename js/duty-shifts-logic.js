        ...
        function findLastWeekendBeforeInRange(sortedWeekends, thresholdKey, calcStartKey, calcEndKey) {
            let lastWk = null;
            if (!Array.isArray(sortedWeekends)) return null;
            for (const wk of sortedWeekends) {
                if (wk >= thresholdKey) break;
                if (wk >= calcStartKey && wk <= calcEndKey) lastWk = wk;
            }
            return lastWk;
        }

        /**
         * Build per-date weekend baseline holders (rotation slot), matching preview: month reseed,
         * manual-alternate defer, and cursor advance — not naive pos++.
         */
        function buildWeekendBaselineByDateForRotation(sortedWeekends) {
            const baselineWeekendByDate = {};
            if (!Array.isArray(sortedWeekends) || sortedWeekends.length === 0) return baselineWeekendByDate;
            const globalPos = {};
            const deferState = {};
            let prevCalMonthKey = null;
            for (const dk of sortedWeekends) {
                ...
                if (prevCalMonthKey !== monthKey) {
                    for (const k of Object.keys(deferState)) delete deferState[k];
                    if (typeof seedManualAlternateDeferAllGroupsForMonthStart === 'function') {
                        seedManualAlternateDeferAllGroupsForMonthStart(deferState, 'weekend', date);
                    }
                    for (let g = 1; g <= 4; g++) {
                        const grp =
                            (typeof groupsForDuty === 'function' ? groupsForDuty(g, dk) : groups[g]) || {
                                weekend: []
                            };
                        const people = grp.weekend || [];
                        if (people.length) {
                            reseedGlobalRotationPositionAtMonthStart('weekend', date, g, people, globalPos);
                        }
                    }
                    prevCalMonthKey = monthKey;
                }
                ...
                for (let g = 1; g <= 4; g++) {
                    ...
                    const rotationDays = people.length;
                    if (globalPos[g] === undefined) {
                        reseedGlobalRotationPositionAtMonthStart('weekend', date, g, people, globalPos);
                    }
                    const resolved = resolveRotationPositionWithManualAlternate(
                        g,
                        people,
                        rotationDays,
                        globalPos,
                    ...
                    const rotationPosition = resolved.rotationPosition;
                    const rotationPerson = people[rotationPosition];
                    if (rotationPerson) baselineWeekendByDate[dk][g] = rotationPerson;
                    const assignedForAdvance = applyManualAlternateToAssignedPerson(
                        rotationPerson,
                        resolved.existingManualAlternate,
                        false
                    );
                    if (assignedForAdvance) {
                        advanceGlobalRotationAfterDutyAssignment(
                            ...
                            deferState,
                            {
                                ...
                                existingManualAlternate: resolved.existingManualAlternate
                            }
                        );
                    } else {
                        globalPos[g] = (rotationPosition + 1) % rotationDays;
                    }
                }
            }
            return baselineWeekendByDate;
        }

        function getStoredWeekendBaselinePersonForGroup(dateKey, groupNum) {
            if (!dateKey || !Number.isFinite(groupNum)) return null;
            const baseline =
                typeof getRotationBaselineAssignmentForType === 'function'
                    ? getRotationBaselineAssignmentForType('weekend', dateKey)
                    : null;
            if (!baseline) return null;
            return typeof parseAssignedPersonForGroupFromAssignment === 'function'
                ? parseAssignedPersonForGroupFromAssignment(baseline, groupNum)
                : null;
        }

        /** First eligible weekend on/after threshold within calc range (prefer same month as threshold). */
        function pickWeekendReturnFromMissingTargetKey(sortedWeekends, thirdDayAfterEnd, calcStartKey, calcEndKey, occupiedMap, groupNum) {
            if (!thirdDayAfterEnd || !Array.isArray(sortedWeekends) || sortedWeekends.length === 0 || !calcStartKey || !calcEndKey) {
                return null;
            }
            ...
            for (const wk of sortedWeekends) {
                ...
                if (wk.substring(0, 7) !== monthPrefix) break;
                const t = tryKey(wk);
                if (t) return t;
            }
            for (const wk of sortedWeekends) {
        ...
        function resolveWeekendReturnFromMissingTarget(sortedWeekends, calcStartKey, calcEndKey, pStartKey, pEndKey, occupiedMap, groupNum) {
            ...
            if (!targetWeekendKey) {
                ...
                backwardCandidates.sort((a, b) => (a < b ? 1 : a > b ? -1 : 0));
                for (const cand of backwardCandidates) {
                    if (!occupiedMap?.[cand]?.[groupNum]) {
                        targetWeekendKey = cand;
            ...
            if (occupiedMap?.[targetWeekendKey]?.[groupNum]) {
                for (let i = weekendIdx + 1; i < sortedWeekends.length; i++) {
                    const wk = sortedWeekends[i];
                    if (wk < calcStartKey || wk > calcEndKey) break;
                    if (!occupiedMap[wk]?.[groupNum]) {
                        targetWeekendKey = wk;
                        break;
                    }
                }
            ...
            if (!targetWeekendKey || occupiedMap?.[targetWeekendKey]?.[groupNum]) {
                return { targetWeekendKey: null, isBackwardAssignment: false };
            }
            return { targetWeekendKey, isBackwardAssignment };
        }

        /**
         * Build weekend return-from-missing targets for a calculation range (same-month forward/backward like normal/semi).
         */
        function buildWeekendReturnFromMissingTargetsMap(sortedWeekends, calcStartKey, calcEndKey, baselineWeekendByDate) {
            const out = {};
            if (!calcStartKey || !calcEndKey || !Array.isArray(sortedWeekends) || sortedWeekends.length === 0) return out;
            ...
            const calcStartDateObj = new Date(calcStartKey + 'T00:00:00');
            const prevMonthStart = new Date(calcStartDateObj.getFullYear(), calcStartDateObj.getMonth() - 1, 1);
            ...
            for (let groupNum = 1; groupNum <= 4; groupNum++) {
                ...
                const weekendList =
                    typeof getSortedGroupListForRotation === 'function'
                        ...
                        : g?.weekend || [];
                const resolveWeekendPersonKey = (key) => {
                    const nk = normW(key);
                ...
                for (const personNameRaw of Object.keys(missingMap)) {
                    ...
                    for (const period of periods) {
                        ...
                        const returnKeyForRange = addDaysToDateKeyLocal(pEndKey, 1);
                        const returnInRange =
                        ...
                        if (!hadMissedWeekend && periodOverlapsRange) {
                            for (const wk of sortedWeekends) {
                                if (wk < scanStartKey || wk > scanEndKey) continue;
                                const stored = getStoredWeekendBaselinePersonForGroup(wk, groupNum);
                                if (stored && normW(stored) === normW(personName)) {
                                    if (!firstMissedWeekendKey) firstMissedWeekendKey = wk;
                                    hadMissedWeekend = true;
                                    break;
                                }
                        ...
                        if (!out[targetWeekendKey]) out[targetWeekendKey] = {};
                        out[targetWeekendKey][groupNum] = {
                            personName,
                            missingEnd: pEndKey,
                            missingStart: pStartKey,
                            firstMissedWeekendKey: firstMissedWeekendKey || null,
                            isBackwardAssignment,
                            missingRangeStr: formatDDMMYYYYW(pStartKey) + ' - ' + formatDDMMYYYYW(pEndKey),
                            reasonOfMissing: (period?.reason || '').trim() || '(δεν αναφέρεται λόγος)'
                        };
                    }
                }
            }
        ...
        function applyWeekendReturnFromMissingTargetsToAssignments(
            returnFromMissingWeekendTargets,
            updatedAssignments,
            baselineWeekendByDate
        ) {
            if (!returnFromMissingWeekendTargets || !updatedAssignments) return;
            const normW = (s) => (typeof normalizePersonKey === 'function' ? normalizePersonKey(s) : String(s || '').trim());
            for (const dateKey of Object.keys(returnFromMissingWeekendTargets)) {
                const byGroup = returnFromMissingWeekendTargets[dateKey];
                if (!byGroup) continue;
                const date = new Date(dateKey + 'T00:00:00');
                for (const groupNumStr of Object.keys(byGroup)) {
                    const groupNum = parseInt(groupNumStr, 10);
                    const designated = byGroup[groupNum];
                    if (!designated?.personName || !Number.isFinite(groupNum)) continue;
                    const groupData =
                    ...
                    updatedAssignments[dateKey][groupNum] = matchingPerson;
                    const reasonText =
                        typeof buildReturnFromMissingPlacementUnifiedMessage === 'function' &&
                        designated.firstMissedWeekendKey &&
                        displacedPerson
                            ? buildReturnFromMissingPlacementUnifiedMessage({
                                  returningPersonName: matchingPerson,
                                  displacedPersonName: displacedPerson,
                                  placementDateKey: dateKey,
                                  baselineDateKey: designated.firstMissedWeekendKey,
                                  missingStartKey: designated.missingStart || null,
                                  missingEndKey: designated.missingEnd,
                                  reasonOfMissing: designated.reasonOfMissing
                              })
                            : `Τοποθετήθηκε σε υπηρεσία γιατί θα απουσιάζει (${designated.missingRangeStr || ''}) λόγω ${designated.reasonOfMissing || '(δεν αναφέρεται λόγος)'}`;
                    storeAssignmentReason(dateKey, groupNum, matchingPerson, 'skip', reasonText, displacedPerson, null, {
                        returnFromMissing: true,
                        insertedByShift: true,
                        missingEnd: designated.missingEnd,
                        missingStart: designated.missingStart || null,
                        baselineDateKey: designated.firstMissedWeekendKey || null,
                        reasonOfMissing: designated.reasonOfMissing,
        ...
        function findExistingManualAlternateOnDateGroup(dateKey, groupNum) {
            ...
            for (const pn in gReasons) {
                ...
                if (r && r.type === 'skip' && r.meta && r.meta.manualAlternateReplacement && r.swappedWith) {
                    return { replacement: pn, baseline: r.swappedWith };
                }
            }
            return null;
        }
         ...
         */
        function resolveRotationPositionWithManualAlternate(
            groupNum,
            groupPeople,
            rotationDays,
            ...
            let rotationPosition = Number.isFinite(globalRotationPosition[groupNum])
                ...
                : 0;
            const norm = (s) => (typeof normalizePersonKey === 'function' ? normalizePersonKey(s) : String(s || '').trim());
            let deferFulfillment = null;
            const dutyCat =
                dayTypeCategory ||
                (typeof getDutyCategoryForDateKeyLocal === 'function' ? getDutyCategoryForDateKeyLocal(dateKey) : null) ||
                (typeof getDutyCategoryForDateKey === 'function' ? getDutyCategoryForDateKey(dateKey) : 'normal');
            ...
            if (existingManualAlternate) {
                ...
                if (st?.person) {
                    ...
                    const atDeferPerson =
                        groupPeople[rotationPosition] && norm(groupPeople[rotationPosition]) === dn;
                    if (atDeferPerson && dutyCat) {
                        const dateInMonth = new Date(dateKey + 'T00:00:00');
                        const latest =
                            typeof getManualAlternateDeferFromPreviousMonth === 'function'
                                ? getManualAlternateDeferFromPreviousMonth(dutyCat, dateInMonth, groupNum)
                                : typeof findLatestManualAlternateInPreviousMonth === 'function'
                                  ? findLatestManualAlternateInPreviousMonth(dutyCat, dateInMonth, groupNum)
                                  : null;
                        deferFulfillment = {
                            skippedReplacement: st.person,
                            baselinePerson: latest?.baselinePerson || null,
                            sourceDateKey: latest?.dateKey || null,
                            prevMonthKey:
                                typeof getPreviousMonthKeyFromDate === 'function'
                                    ? getPreviousMonthKeyFromDate(dateInMonth)
                                    : null
                        };
                    }
                    let guard = 0;
        ...
        function isManualAlternateReplacementOnDate(dateKey, groupNum, assignedPerson) {
            const reason = typeof getAssignmentReason === 'function'
                ? getAssignmentReason(dateKey, groupNum, assignedPerson)
                : null;
            return !!(reason && reason.type === 'skip' && reason.meta && reason.meta.manualAlternateReplacement);
        }

        function advanceGlobalRotationAfterDutyAssignment(
            groupNum,
            groupPeople,
            rotationDays,
            rotationPosition,
            ...
            if (wasReplaced && replacementIndex !== null && assignedPerson) {
                globalRotationPosition[groupNum] = (replacementIndex + 1) % rotationDays;
            } else if (assignedPerson) {
                const assignedIndex = groupPeople.indexOf(assignedPerson);
                if (assignedIndex !== -1) {
                    globalRotationPosition[groupNum] = (assignedIndex + 1) % rotationDays;
                } else {
                    globalRotationPosition[groupNum] = (rotationPosition + 1) % rotationDays;
                }
            } else {
        ...
        async function calculateDutiesForSelectedMonths() {
            ...
            try {
                ...
                if (!startMonthInput) {
                    ...
                    console.error('Missing form elements:', { startMonthInput });
                    return;
                }
        ...
        async function runWeekendSkipLogic() {
            try {
                ...
                // Return-from-missing (weekend): place absent person on another holiday in same calc month before/after absence.
                const calcStartDateW = calculationSteps.startDate;
                const calcEndDateW = calculationSteps.endDate;
                const calcStartKeyW =
                    calcStartDateW && !isNaN(calcStartDateW.getTime()) ? formatDateKey(calcStartDateW) : null;
                const calcEndKeyW = calcEndDateW && !isNaN(calcEndDateW.getTime()) ? formatDateKey(calcEndDateW) : null;
                if (calcStartKeyW && calcEndKeyW && sortedWeekends.length > 0) {
                    const baselineWeekendByDateSkip = buildWeekendBaselineByDateForRotation(sortedWeekends);
                    const returnFromMissingWeekendTargetsSkip = buildWeekendReturnFromMissingTargetsMap(
                        sortedWeekends,
                        calcStartKeyW,
                        calcEndKeyW,
                        baselineWeekendByDateSkip
                    );
                    applyWeekendReturnFromMissingTargetsToAssignments(
                        returnFromMissingWeekendTargetsSkip,
                        updatedAssignments,
                        baselineWeekendByDateSkip
                    );
                }
                ...
                for (const dk in assignmentReasons) {
                    ...
                    for (const gk in byGroup) {
                        ...
                        for (const pname in gReasons) {
                            ...
                            // Normal-day swap defer applies only to manual alternates on normal days (not semi/weekend).
                            const dkDate = new Date(dk + 'T00:00:00');
                            if (!isNaN(dkDate.getTime()) && getDayType(dkDate) !== 'normal-day') continue;
                        ...
                        if (hasConsecutiveConflict) {
                            ...
                            if (!swapFound && applyWeekPairLogic && (dayOfWeek === 1 || dayOfWeek === 3)) {
                                ...
                                if (!swapFound) {
                                    ...
                                    for (const offset of backwardOffsets) {
                                        ...
                                        const result = tryBackwardSwapCandidate(prevDayKey);
                                        if (result) {
                                ...
                                // MONDAY/WEDNESDAY - Step 3: ONLY if Step 2b failed, try alternative day in week after next OR next month
                                if (!swapFound) {
                            ...
                            // TUESDAY/THURSDAY - Separate logic block
                                ...
                                if (normalDays.includes(nextSameDayKey) && updatedAssignments[nextSameDayKey]?.[groupNum]) {
                                    ...
                                    // Next same day is in next month - DON'T try cross-month yet! Try backward swaps first.
                                } else {
                                }
        ...
        function renderStep2_Weekends() {
            ...
            if (weekendHolidays.length === 0) {
                ...
                // Baseline weekend rotation (manual-alternate aware) for return-from-missing detection
                const baselineWeekendByDate = buildWeekendBaselineByDateForRotation(sortedWeekends);
                
                // Return-from-missing for weekend: same calculated month (forward after 3 days, else backward before absence start).
                const calcStartKeyW =
                    startDate && !isNaN(new Date(startDate).getTime()) ? formatDateKey(new Date(startDate)) : null;
                const calcEndKeyW = endDate && !isNaN(new Date(endDate).getTime()) ? formatDateKey(new Date(endDate)) : null;
                const returnFromMissingWeekendTargets =
                    calcStartKeyW && calcEndKeyW && sortedWeekends.length > 0
                        ? buildWeekendReturnFromMissingTargetsMap(
                              sortedWeekends,
                              calcStartKeyW,
                              calcEndKeyW,
                              baselineWeekendByDate
                    ...
                    for (let groupNum = 1; groupNum <= 4; groupNum++) {
                        ...
                        if (groupPeople.length === 0) {
                            ...
                            if (
                                ...
                                const reasonText =
                                    typeof buildReturnFromMissingPlacementUnifiedMessage === 'function' &&
                                    designatedWeekend.firstMissedWeekendKey &&
                                    displacedPerson
                                        ? buildReturnFromMissingPlacementUnifiedMessage({
                                              returningPersonName: assignedPerson,
                                              displacedPersonName: displacedPerson,
                                              placementDateKey: dateKey,
                                              baselineDateKey: designatedWeekend.firstMissedWeekendKey,
                                              missingStartKey: designatedWeekend.missingStart || null,
                                              missingEndKey: designatedWeekend.missingEnd,
                                              reasonOfMissing: designatedWeekend.reasonOfMissing
                                          })
                                ...
                                storeAssignmentReason(dateKey, groupNum, assignedPerson, 'skip', reasonText, displacedPerson, null, {
                                    returnFromMissing: true,
                                    insertedByShift: true,
                                    missingEnd: designatedWeekend.missingEnd,
                                    missingStart: designatedWeekend.missingStart || null,
                                    baselineDateKey: designatedWeekend.firstMissedWeekendKey || null,
                                    reasonOfMissing: designatedWeekend.reasonOfMissing,
                            ...
                            const manualAltResolved = resolveRotationPositionWithManualAlternate(
                                groupNum,
                                groupPeople,
                                rotationDays,
                                globalWeekendRotationPosition,
                                deferManualAlternateSkipWeekend,
                                dateKey,
                                'weekend'
                            );
                            let rotationPosition = manualAltResolved.rotationPosition;
                            const existingManualAlternateWeekend = manualAltResolved.existingManualAlternate;
                            ...
                            if (isRotationPersonDisabledWeekend) {
                                ...
                                for (let offset = 1; offset <= rotationDays * 2 && !foundEligible; offset++) {
                                    ...
                                    replacementIndex = idx;
                                    wasReplaced = true;
                                    wasDisabledOnlySkippedWeekend = true;
                                    foundEligible = true;
                            ...
                            if (assignedPerson) {
                                ...
                                if (
                                    manualAltResolved.deferFulfillment &&
                                    !wasDisabledOnlySkippedWeekend &&
                                    tryStoreManualAlternateDeferFulfillmentReason(
                                        dateKey,
                                        groupNum,
                ...
                for (let g = 1; g <= 4; g++) {
                    ...
                    for (let i = sortedWeekendKeys.length - 1; i >= 0; i--) {
                        ...
                        const ap = simulatedWeekendAssignments[dateKey]?.[g];
                        if (!ap) continue;
                        lastPersonForRotation = getPersonForRotationContinuity(dateKey, g, ap, simulatedWeekendAssignments);
                        break;
                    }
                    if (lastPersonForRotation) {
                        calculationSteps.lastWeekendRotationPositions[g] = lastPersonForRotation;
                        console.log(`[WEEKEND ROTATION] Storing last rotation continuity person ${lastPersonForRotation} for group ${g}`);
                    }
                }

                const sortedWeekendKeysForMonth = [...weekendHolidays].sort();
                const lastWeekendRotationPositionsByMonth = {};
                for (const dateKey of sortedWeekendKeysForMonth) {
                    ...
                    for (let g = 1; g <= 4; g++) {
                        ...
                        if (assignedPerson) {
                            const personForRotation = getPersonForRotationContinuity(dateKey, g, assignedPerson, simulatedWeekendAssignments);
                            if (!lastWeekendRotationPositionsByMonth[monthKey]) {
                                lastWeekendRotationPositionsByMonth[monthKey] = {};
                            }
                            lastWeekendRotationPositionsByMonth[monthKey][g] = personForRotation;
                        }
                    }
                }
                    ...
                    if (existingManualAlternateSemiRun) {
                        ...
                        if (stDefer?.person) {
                            ...
                            if (groupPeople[pos] && normSemiRun(groupPeople[pos]) === dn) {
                                const latestSemi =
                                    typeof getManualAlternateDeferFromPreviousMonth === 'function'
                                        ? getManualAlternateDeferFromPreviousMonth('semi', date, groupNum)
                                        : null;
                                deferFulfillmentSemiRun = {
                                    skippedReplacement: stDefer.person,
                                    baselinePerson: latestSemi?.baselinePerson || null,
                                    sourceDateKey: latestSemi?.dateKey || null,
                                    prevMonthKey:
                                        typeof getPreviousMonthKeyFromDate === 'function'
                                            ? getPreviousMonthKeyFromDate(date)
                                            : null
                                };
                            }
                            let guard = 0;
            ...
            if (semiNormalDays.length === 0) {
                ...
                sortedSemi.forEach((dateKey, semiIndex) => {
                    ...
                    for (let groupNum = 1; groupNum <= 4; groupNum++) {
                        ...
                        if (groupPeople.length === 0) {
                            ...
                            const manualAltResolvedSemi = resolveRotationPositionWithManualAlternate(
                                groupNum,
                                groupPeople,
                                rotationDays,
                                globalSemiRotationPosition,
                                deferManualAlternateSkipSemi,
                                dateKey,
                                'semi'
                            );
                            ...
                            if (existingManualAlternateSemi && assignedPerson && !wasDisabledOnlySkippedSemi) {
                                ...
                                if (normS(assignedPerson) === normS(existingManualAlternateSemi.replacement)) {
                                    semiRotationPersons[dateKey][groupNum] = existingManualAlternateSemi.baseline;
                                } else {
                            ...
                            if (
                                assignedPerson &&
                                manualAltResolvedSemi.deferFulfillment &&
                                !wasDisabledOnlySkippedSemi &&
                                tryStoreManualAlternateDeferFulfillmentReason(
                                    dateKey,
                                    groupNum,
                                ...
                                semiRotationPersons[dateKey][groupNum] =
                                    manualAltResolvedSemi.deferFulfillment.skippedReplacement;
                            }
                ...
                for (let g = 1; g <= 4; g++) {
                    ...
                    for (let i = sortedSemiKeys.length - 1; i >= 0; i--) {
                        ...
                        const ap = semiAssignments[dateKey]?.[g];
                        if (!ap) continue;
                        lastPersonForRotation = getPersonForRotationContinuity(dateKey, g, ap, semiAssignments);
                        break;
                    }
                    if (lastPersonForRotation) {
                        calculationSteps.lastSemiRotationPositions[g] = lastPersonForRotation;
                        console.log(`[SEMI ROTATION] Storing last rotation continuity person ${lastPersonForRotation} for group ${g}`);
                    }
                }
                ...
                for (const dateKey of sortedSemiKeysForMonth) {
                    ...
                    for (let g = 1; g <= 4; g++) {
                        ...
                        if (assignedPerson) {
                            const personForRotation = getPersonForRotationContinuity(dateKey, g, assignedPerson, semiAssignments);
                            if (!lastSemiRotationPositionsByMonth[monthKey]) {
                                lastSemiRotationPositionsByMonth[monthKey] = {};
                            }
                            lastSemiRotationPositionsByMonth[monthKey][g] = personForRotation;
                        }
                    }
                }
                calculationSteps.lastSemiRotationPositionsByMonth = lastSemiRotationPositionsByMonth;
                ...
                sortedNormal.forEach((dateKey, normalIndex) => {
                    ...
                    for (let groupNum = 1; groupNum <= 4; groupNum++) {
                        ...
                        if (groupPeople.length === 0) {
                            ...
                            const manualAltResolvedNormal = resolveRotationPositionWithManualAlternate(
                                groupNum,
                                groupPeople,
                                rotationDays,
                                globalNormalRotationPosition,
                            ...
                            let rotationPosition = manualAltResolvedNormal.rotationPosition % rotationDays;
                            existingManualAlternate = manualAltResolvedNormal.existingManualAlternate || existingManualAlternate;
                            const deferFulfillmentNormal = manualAltResolvedNormal.deferFulfillment;
                            ...
                            if (
                                ...
                                if (!normalRotationPersons[dateKey]) normalRotationPersons[dateKey] = {};
                                normalRotationPersons[dateKey][groupNum] = deferFulfillmentNormal.skippedReplacement;
                            }
                                    ...
                                    if (laterOriginalIdx >= 0) {
                                        ...
                                        if (laterPos >= 0) {
                                            for (let ni = laterPos + 1; ni < sortedNormal.length; ni++) {
                                                ...
                                                const nextDateObj = new Date(nextKey + 'T00:00:00');
                                                if (isNaN(nextDateObj.getTime())) continue;
                                                const existingPerson = normalAssignments?.[nextKey]?.[groupNum] || null;
                                                const existingReason = existingPerson ? getAssignmentReason(nextKey, groupNum, existingPerson) : null;
                                                if (existingReason && existingReason.meta && (existingReason.meta.returnFromMissing || existingReason.meta.shiftedByReturnFromMissing)) {
                                                    const existingIdx = groupPeople.indexOf(existingPerson);
                                                    if (existingIdx >= 0) cursorIdx = (existingIdx + 1) % groupPeople.length;
