        ...
        function purgeStaleReplacementReasons(dateKey, groupNum, assignedPerson, expectedSkippedBaseline) {
            ...
            const skippedNorm = expectedSkippedBaseline ? norm(expectedSkippedBaseline) : '';
            const onAssignee = getAssignmentReason(dateKey, groupNum, assignedPerson);
            if (
                onAssignee &&
                isUnavailableReplacementReason(onAssignee) &&
                skippedNorm &&
                onAssignee.swappedWith &&
                norm(onAssignee.swappedWith) !== skippedNorm
            ) {
            ...
            for (const pn of Object.keys(gmap)) {
                ...
                if (!isUnavailableReplacementReason(gmap[pn])) continue;
        ...
        function syncDutyReplacementReason(dateKey, groupNum, baselinePerson, assignedPerson, dateObj, dutyCategory) {
            ...
            if (
                existing &&
                isUnavailableReplacementReason(existing) &&
                existing.swappedWith &&
                norm(existing.swappedWith) === norm(baselinePerson)
            ) {
                return;
            }
        ...
        function mergeDutyBaselineMaps(rotationBaseline, previewSlotBaseline) {
            ...
            const keys = new Set([
                ...
                ...Object.keys(previewSlotBaseline || {})
            ]);
            for (const dk of keys) {
                merged[dk] = { ...(previewSlotBaseline?.[dk] || {}) };
                if (rotationBaseline?.[dk]) Object.assign(merged[dk], rotationBaseline[dk]);
            }
            return merged;
        }
            ...
            try {
                ...
                try {
                    if (calcStartDate && calcEndDate && Array.isArray(sortedNormal) && sortedNormal.length > 0) {
                        ...
                        const usedReturnFromMissingTargets = new Set(); // "dateKey:groupNum" already used by return-from-missing in this run
                                    ...
                                    const assignmentReasonText =
                                        typeof buildReturnFromMissingPlacementUnifiedMessage === 'function'
                                            ? buildReturnFromMissingPlacementUnifiedMessage({
                                                  ...
                                                  displacedPersonName: ins.baselineDisplaced || null,
                ...
                sortedNormal.forEach((dateKey) => {
                    ...
                    for (let groupNum = 1; groupNum <= 4; groupNum++) {
                        ...
                        if (hasConsecutiveConflict) {
                            ...
                            if (!swapFound && applyWeekPairLogic && (dayOfWeek === 1 || dayOfWeek === 3)) {
                                ...
                                if (!swapFound) {
                                    const backwardOffsets =
                                        (typeof isNightChangesGroup === 'function' && isNightChangesGroup(groupNum) && dayOfWeek === 2)
                                            ? [7, 14, 21]
                                            : [7, 2, 9, 14, 16, 21];
                            ...
                            else if (!swapFound && applyWeekPairLogic && (dayOfWeek === 2 || (dayOfWeek === 4 && !(typeof isNightChangesGroup === 'function' && isNightChangesGroup(groupNum))))) {
                ...
                if (Object.keys(updatedAssignments).length > 0) {
                    ...
                    for (const dateKey of [...allRotationDateKeys].sort()) {
                        ...
                        for (let groupNum = 1; groupNum <= 4; groupNum++) {
                            ...
                            if (!assignedPerson) {
                                assignedPerson = baselineByDateForRotation[dateKey]?.[groupNum] ||
                                    baselineByDateForRotation[dateKey]?.[String(groupNum)] ||
                                    null;
                            }
                    ...
                    const reasonText =
                        typeof buildReturnFromMissingPlacementUnifiedMessage === 'function'
                            ? buildReturnFromMissingPlacementUnifiedMessage({
                                  ...
                                  displacedPersonName: baselineDisplaced,
                                  placementDateKey: dateKey,
