        ...
        function hasConsecutiveDuty(dayKey, person, groupNum, simulatedAssignments = null) {
            ...
            // Only check day after if it's already assigned (exists in any assignment document or simulated)
        ...
        function assignDutiesForDayType(dayTypeCategory, groupNum, groupData, dayTypeLists) {
            ...
            let groupPeople;
                    ...
                    days.forEach((dayKey, dayIndex) => {
                        ...
                        // First, try to load from lastRotationPositions (saved from previous calculation)
                        let rotationPosition;
                        ...
                        if (dayTypeCategory === 'weekend') {
                            ...
                            if (globalWeekendRotationPosition[groupNum] === undefined) {
                                ...
                                if (isFebruary2026) {
                                    globalWeekendRotationPosition[groupNum] = 0;
                                    console.log(`[ROTATION] Starting from first person (position 0) for group ${groupNum} weekend - February 2026`);
                                } else if (lastRotationPositions.weekend[groupNum] !== undefined) {
                                    globalWeekendRotationPosition[groupNum] = lastRotationPositions.weekend[groupNum];
                                    console.log(`[ROTATION] Loaded weekend rotation position for group ${groupNum} from Firestore: ${globalWeekendRotationPosition[groupNum]}`);
                                } else {
                                    // Initialize based on rotation count from start date
                            ...
                            if (globalNormalRotationPosition[groupNum] === undefined) {
                                ...
                                if (isFebruary2026) {
                                    globalNormalRotationPosition[groupNum] = 0;
                                    console.log(`[ROTATION] Starting from first person (position 0) for group ${groupNum} - February 2026`);
                                } else if (lastRotationPositions.normal[groupNum] !== undefined) {
                                    globalNormalRotationPosition[groupNum] = lastRotationPositions.normal[groupNum];
                                    console.log(`[ROTATION] Loaded normal rotation position for group ${groupNum} from Firestore: ${globalNormalRotationPosition[groupNum]}`);
                                } else {
                                    // Initialize based on rotation count from start date
                                    const daysSinceStart = getRotationPosition(dayDate, dayTypeCategory, groupNum);
                                    globalNormalRotationPosition[groupNum] = daysSinceStart % rotationDays;
                            ...
                            if (globalSemiRotationPosition[groupNum] === undefined) {
                                ...
                                if (isFebruary2026) {
                                    globalSemiRotationPosition[groupNum] = 0;
                                    console.log(`[ROTATION] Starting from first person (position 0) for group ${groupNum} semi - February 2026`);
                                } else if (lastRotationPositions.semi[groupNum] !== undefined) {
                                    globalSemiRotationPosition[groupNum] = lastRotationPositions.semi[groupNum];
                                    console.log(`[ROTATION] Loaded semi rotation position for group ${groupNum} from Firestore: ${globalSemiRotationPosition[groupNum]}`);
                                } else {
                                    // Initialize based on rotation count from start date
                                    const daysSinceStart = getRotationPosition(dayDate, dayTypeCategory, groupNum);
                                    globalSemiRotationPosition[groupNum] = daysSinceStart % rotationDays;
                            ...
                            if (globalSpecialRotationPosition[groupNum] === undefined) {
                                ...
                                if (isFebruary2026) {
                                    ...
                                    console.log(`[ROTATION] Starting from first person (position 0) for group ${groupNum} special - February 2026`);
                                } else if (lastRotationPositions.special[groupNum] !== undefined) {
                                    globalSpecialRotationPosition[groupNum] = lastRotationPositions.special[groupNum];
                                    console.log(`[ROTATION] Loaded special rotation position for group ${groupNum} from Firestore: ${globalSpecialRotationPosition[groupNum]}`);
                                } else {
                                    // Initialize based on rotation count from start date
                                    const daysSinceStart = getRotationPosition(dayDate, dayTypeCategory, groupNum);
                        ...
                        if (dayTypeCategory === 'normal' && expectedPerson && assignedPeople[monthKey] && assignedPeople[monthKey][groupNum] && assignedPeople[monthKey][groupNum].has(expectedPerson)) {
                            ...
                            if (!foundReplacement) {
                                ...
                                if (daysUntilEndOfMonth <= 3) {
                                    ...
                                    if (nextMonthResult && nextMonthResult.person) {
                                        ...
                                        // Store cross-month swap: the person with conflict (expectedPerson before swap) must be assigned to swapDayKey in next month
                                        // Note: We need to track the original expectedPerson before it was replaced with nextMonthResult.person
                                        // This will be handled in the swap logic below where skippedPerson is available
                                        // For now, we'll track it in a variable that will be used when skippedPerson is set
                                        const originalExpectedPersonBeforeSwap = expectedPerson; // Store before replacement
                                        // This will be saved when skippedPerson is set in the conflict handling below
                                    }
                                }
                                ...
                                if (!foundReplacement) {
                                    ...
                                    if (dayTypeCategory === 'normal') {
                                        ...
                                        lastRotationPositions.weekend[groupNum] = nextRotationPosition;
                                    } else if (dayTypeCategory === 'semi') {
                                        globalSemiRotationPosition[groupNum] = nextRotationPosition;
                                        lastRotationPositions.semi[groupNum] = nextRotationPosition;
                                    } else if (dayTypeCategory === 'special') {
                                        globalSpecialRotationPosition[groupNum] = nextRotationPosition;
                                        lastRotationPositions.special[groupNum] = nextRotationPosition;
                                    }
                                    return;
                                }
                            }
                            ...
                            // Note: We don't update lastRotationPositions here because we want to track the "normal" rotation
                            // (the expected person's position), not the replacement's position
                            if (dayTypeCategory === 'normal') {
                                globalNormalRotationPosition[groupNum] = rotationPosition;
                            } else if (dayTypeCategory === 'weekend') {
                                globalWeekendRotationPosition[groupNum] = rotationPosition;
                            } else if (dayTypeCategory === 'semi') {
                                globalSemiRotationPosition[groupNum] = rotationPosition;
                            } else if (dayTypeCategory === 'special') {
                                globalSpecialRotationPosition[groupNum] = rotationPosition;
                            }
                        }
                        ...
                        if (expectedPerson && !isPersonMissingOnDate(expectedPerson, groupNum, dayDate)) {
                            ...
                            if (dayTypeCategory === 'normal' && swappedPeople.has(expectedPerson)) {
                                ...
                                // Advance rotation position for next iteration (continue globally, ignoring swaps)
                                // This is the "normal" rotation position (what it would be if no swaps occurred)
                                const nextRotationPosition = (rotationPosition + 1) % rotationDays;
                                if (dayTypeCategory === 'normal') {
                                    globalNormalRotationPosition[groupNum] = nextRotationPosition;
                            ...
                            // If expected person has NO conflicts, assign them
                                ...
                                if (dayTypeCategory === 'weekend') {
                                    ...
                                    storeAssignmentReason(dayKey, groupNum, skippedPerson, 'skip', reason);
                                        ...
                                        assignedPerson = expectedPerson;
                                        // Advance rotation position (normal rotation, ignoring swaps)
                                        const nextRotationPosition = (rotationPosition + 1) % rotationDays;
                                        if (dayTypeCategory === 'weekend') {
                                            globalWeekendRotationPosition[groupNum] = nextRotationPosition;
                                            lastRotationPositions.weekend[groupNum] = nextRotationPosition;
                                    ...
                                    if (swapDayKey !== null && swapDayIndex !== null) {
                                        ...
                                        if (swapPerson && !isPersonMissingOnDate(swapPerson, groupNum, dayDate) && 
                                            ...
                                            if (userApproved) {
                                                ...
                                                // Check if swap day is in next month - if so, save cross-month swap
                                                const swapDate = new Date(swapDayKey + 'T00:00:00');
                                                const swapMonth = swapDate.getMonth();
                                                const swapYear = swapDate.getFullYear();
                                                if (swapMonth !== month || swapYear !== year) {
                                                    // Swap day is in next month - save cross-month swap
                                                    if (!crossMonthSwaps[swapDayKey]) {
                                                        crossMonthSwaps[swapDayKey] = {};
                                                    }
                                                    crossMonthSwaps[swapDayKey][groupNum] = skippedPerson;
                                                    console.log(`[CROSS-MONTH SWAP] Person ${skippedPerson} (had conflict on ${dayKey}) must be assigned to ${swapDayKey} (Group ${groupNum})`);
                                                }
                                                ...
                                                if (dayTypeCategory === 'normal') {
                                                    globalNormalRotationPosition[groupNum] = nextRotationPosition;
                                                    // Track last rotation position (normal rotation, ignoring swaps)
                                                    lastRotationPositions.normal[groupNum] = nextRotationPosition;
                                                } else if (dayTypeCategory === 'weekend') {
                                                    globalWeekendRotationPosition[groupNum] = nextRotationPosition;
                                                    lastRotationPositions.weekend[groupNum] = nextRotationPosition;
                                    ...
                                    if (!foundReplacement) {
                                        ...
                                        if (nextMonthResult && nextMonthResult.person) {
                                            ...
                                            if (nextMonthResult.swapDayKey && skippedPerson) {
                                                ...
                                                crossMonthSwaps[nextMonthResult.swapDayKey][groupNum] = skippedPerson;
                                                console.log(`[CROSS-MONTH SWAP] Person ${skippedPerson} (had conflict on ${dayKey}) must be assigned to ${nextMonthResult.swapDayKey} (Group ${groupNum})`);
                                            }
                                        }
                                    }
