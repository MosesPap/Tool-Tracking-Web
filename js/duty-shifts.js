        ...
        // Optional: assignedPeopleMap - tracks in-progress assignments during calculation loop
        function hasConsecutiveDuty(dayKey, person, groupNum, assignedPeopleMap = null) {
            const date = new Date(dayKey + 'T00:00:00');
            ...
            // Helper function to check if person has duty on a day (checks both saved and in-progress assignments)
            const checkHasDutyOnDay = (checkDayKey) => {
                // First check in-progress assignments if provided (for same-day loop processing)
                if (assignedPeopleMap) {
                    const checkDate = new Date(checkDayKey + 'T00:00:00');
                    const checkMonth = checkDate.getMonth();
                    const checkYear = checkDate.getFullYear();
                    const checkMonthKey = `${checkYear}-${checkMonth}`;
                    
                    if (assignedPeopleMap[checkMonthKey] && assignedPeopleMap[checkMonthKey][groupNum]) {
                        if (assignedPeopleMap[checkMonthKey][groupNum].has(person)) {
                            return true;
                        }
                    }
                }
                
                // Then check saved assignments
                return hasDutyOnDay(checkDayKey, person, groupNum);
            };
            ...
            dayBefore.setDate(dayBefore.getDate() - 1);
            const dayBeforeKey = formatDateKey(dayBefore);
            const hasDutyBefore = checkHasDutyOnDay(dayBeforeKey);
            ...
            // Check all assignment sources for day after (both saved and in-progress)
            ...
            const hasDutyAfter = (dayAfterAssigned && hasDutyOnDay(dayAfterKey, person, groupNum)) || 
                                 checkHasDutyOnDay(dayAfterKey);
            
            // Only check day after if it's already assigned (exists in any assignment document or in-progress)
            // This means it was processed earlier due to higher priority or in the same loop
            if (hasDutyAfter) {
        ...
        function renderStep2_Weekends() {
            ...
            // First, simulate what special holidays will be assigned (from Step 1 logic)
            // This creates a map: monthKey -> groupNum -> array of people with special holidays
                ...
                sortedWeekends.forEach((dateKey, weekendIndex) => {
                    ...
                    for (let groupNum = 1; groupNum <= 4; groupNum++) {
                        ...
                        if (groupPeople.length === 0) {
                            ...
                            // Check if assigned person has special holiday in same month (from simulated assignments)
                            ...
                            const wasSkipped = skippedInMonth[monthKey][groupNum].has(assignedPerson);
                            // Check for consecutive duty using simulated assignments
                            ...
                            if (assignedPerson) {
                                ...
                                // Check if person has duty on day before (from simulated assignments)
                                // Only check special holidays and weekends (semi and normal haven't been simulated yet in Step 2)
                                const hasDutyBefore = simulatedWeekendAssignments[dayBeforeKey]?.[groupNum] === assignedPerson ||
                                                      simulatedSpecialAssignments[`${dayBefore.getFullYear()}-${dayBefore.getMonth()}`]?.[groupNum]?.has(assignedPerson);
                                
                                // Check if person has duty on day after (from simulated assignments)
                                // Only check special holidays and weekends (semi and normal haven't been simulated yet in Step 2)
                                const hasDutyAfter = simulatedWeekendAssignments[dayAfterKey]?.[groupNum] === assignedPerson ||
                                                     simulatedSpecialAssignments[`${dayAfter.getFullYear()}-${dayAfter.getMonth()}`]?.[groupNum]?.has(assignedPerson);
                                
                                if (hasDutyBefore || hasDutyAfter) {
                                    const beforeType = getDayType(dayBefore);
                                    const afterType = getDayType(dayAfter);
                                    const currentType = getDayType(date);
                                    
                                    let beforeTypeCategory = 'normal';
                                    if (beforeType === 'special-holiday') beforeTypeCategory = 'special';
                                    else if (beforeType === 'semi-normal-day') beforeTypeCategory = 'semi';
                                    else if (beforeType === 'weekend-holiday') beforeTypeCategory = 'weekend';
                                    
                                    let afterTypeCategory = 'normal';
                                    if (afterType === 'special-holiday') afterTypeCategory = 'special';
                                    else if (afterType === 'semi-normal-day') afterTypeCategory = 'semi';
                                    else if (afterType === 'weekend-holiday') afterTypeCategory = 'weekend';
                                    
                                    let currentTypeCategory = 'normal';
                                    if (currentType === 'special-holiday') currentTypeCategory = 'special';
                                    else if (currentType === 'semi-normal-day') currentTypeCategory = 'semi';
                                    else if (currentType === 'weekend-holiday') currentTypeCategory = 'weekend';
                                    
                                    // Conflict if different day types OR if either day is special holiday
                                    if ((hasDutyBefore && (currentTypeCategory !== beforeTypeCategory || currentType === 'special-holiday' || beforeType === 'special-holiday')) ||
                                        (hasDutyAfter && (currentTypeCategory !== afterTypeCategory || currentType === 'special-holiday' || afterType === 'special-holiday'))) {
                                        hasConsecutive = true;
                                    }
                                }
                            }
                            ...
                            if (assignedPerson && !isPersonMissingOnDate(assignedPerson, groupNum, date)) {
                                if (hasSpecialHoliday || wasSkipped || hasConsecutive) {
                                    // Mark as skipped
                                    skippedInMonth[monthKey][groupNum].add(assignedPerson);
                                    ...
                                    for (let offset = 1; offset < rotationDays; offset++) {
                                        ...
                                        // Check candidate for consecutive duty
                                        let candidateHasConsecutive = false;
                                        if (candidate) {
                                            const dayBefore = new Date(date);
                                            dayBefore.setDate(dayBefore.getDate() - 1);
                                            const dayAfter = new Date(date);
                                            ...
                                            const candidateHasDutyBefore = simulatedWeekendAssignments[dayBeforeKey]?.[groupNum] === candidate ||
                                                                           simulatedSpecialAssignments[`${dayBefore.getFullYear()}-${dayBefore.getMonth()}`]?.[groupNum]?.has(candidate);
                                            
                                            const candidateHasDutyAfter = simulatedWeekendAssignments[dayAfterKey]?.[groupNum] === candidate ||
                                                                          simulatedSpecialAssignments[`${dayAfter.getFullYear()}-${dayAfter.getMonth()}`]?.[groupNum]?.has(candidate);
                                            
                                            if (candidateHasDutyBefore || candidateHasDutyAfter) {
                                                const beforeType = getDayType(dayBefore);
                                                const afterType = getDayType(dayAfter);
                                                const currentType = getDayType(date);
                                                ...
                                                if ((candidateHasDutyBefore && (currentTypeCategory !== beforeTypeCategory || currentType === 'special-holiday' || beforeType === 'special-holiday')) ||
                                                    (candidateHasDutyAfter && (currentTypeCategory !== afterTypeCategory || currentType === 'special-holiday' || afterType === 'special-holiday'))) {
                                                    candidateHasConsecutive = true;
                                                }
                                            }
                                        }
                                        
                                        if (!candidateHasSpecial && !candidateWasSkipped && !candidateHasConsecutive) {
                                            assignedPerson = candidate;
                                            // Update rotation position to the candidate's index + 1 for next iteration
                                            globalWeekendRotationPosition[groupNum] = nextIndex + 1;
                                            break;
                                        }
                                    }
                                } else {
                                    // Person assigned successfully, advance rotation position
                            ...
                            // Store assigned person in simulatedWeekendAssignments for consecutive day checking
                            if (assignedPerson) {
                ...
                for (let groupNum = 1; groupNum <= 4; groupNum++) {
                    ...
                    if (groupPeople.length > 0) {
                        ...
                        if (assignedPerson) {
                            ...
                            // Check if person has duty on day before (from any simulated assignment)
                            const hasDutyBefore = simulatedWeekendAssignments[dayBeforeKey]?.[groupNum] === assignedPerson ||
                                                  simulatedSpecialAssignments[`${dayBefore.getFullYear()}-${dayBefore.getMonth()}`]?.[groupNum]?.has(assignedPerson) ||
                                                  simulatedSemiAssignments[dayBeforeKey]?.[groupNum] === assignedPerson ||
                                                  normalAssignments[dayBeforeKey]?.[groupNum] === assignedPerson;
                            
                            // Check if person has duty on day after (from any simulated assignment)
                            const hasDutyAfter = simulatedWeekendAssignments[dayAfterKey]?.[groupNum] === assignedPerson ||
                                                 simulatedSpecialAssignments[`${dayAfter.getFullYear()}-${dayAfter.getMonth()}`]?.[groupNum]?.has(assignedPerson) ||
                                                 simulatedSemiAssignments[dayAfterKey]?.[groupNum] === assignedPerson ||
                                                 normalAssignments[dayAfterKey]?.[groupNum] === assignedPerson;
                        ...
                        if (assignedPerson && !isPersonMissingOnDate(assignedPerson, groupNum, date)) {
                            if (hasSpecialHoliday || wasSkipped || hasConsecutive) {
                                ...
                                for (let offset = 1; offset < rotationDays; offset++) {
                                    ...
                                    // Also check candidate for consecutive duty
                                    let candidateHasConsecutive = false;
                                    if (candidate) {
                                        const dayBefore = new Date(date);
                                        dayBefore.setDate(dayBefore.getDate() - 1);
                                        ...
                                        const candidateHasDutyBefore = simulatedWeekendAssignments[dayBeforeKey]?.[groupNum] === candidate ||
                                                                       simulatedSpecialAssignments[`${dayBefore.getFullYear()}-${dayBefore.getMonth()}`]?.[groupNum]?.has(candidate) ||
                                                                       simulatedSemiAssignments[dayBeforeKey]?.[groupNum] === candidate ||
                                                                       normalAssignments[dayBeforeKey]?.[groupNum] === candidate;
                                        
                                        const candidateHasDutyAfter = simulatedWeekendAssignments[dayAfterKey]?.[groupNum] === candidate ||
                                                                      simulatedSpecialAssignments[`${dayAfter.getFullYear()}-${dayAfter.getMonth()}`]?.[groupNum]?.has(candidate) ||
                                                                      simulatedSemiAssignments[dayAfterKey]?.[groupNum] === candidate ||
                                                                      normalAssignments[dayAfterKey]?.[groupNum] === candidate;
                        ...
                        if (groupPeople.length === 0) {
                            ...
                            if (pendingNormalSwaps[monthKey][groupNum] && pendingNormalSwaps[monthKey][groupNum].swapToPosition === rotationPosition) {
                                ...
                                if (assignedPerson && swappedPeoplePreview.has(assignedPerson)) {
                                    ...
                                    // Check for consecutive conflicts using hasConsecutiveDuty logic
                                    // This checks for any consecutive duty (semi-normal, weekend, etc.)
                                    // Check day before and day after for any consecutive duty
                                    const dayBefore = new Date(date);
                                    dayBefore.setDate(dayBefore.getDate() - 1);
                                const dayAfter = new Date(date);
                                dayAfter.setDate(dayAfter.getDate() + 1);
                                ...
                                const afterType = getDayType(dayAfter);
                                    ...
                                    // Legacy check for semi-normal (keep for backward compatibility)
                                    if (!hasConsecutiveConflict && afterType === 'semi-normal-day') {
                                        const personAfter = simulatedSemiAssignments[dayAfterKey]?.[groupNum];
                                        if (personAfter === assignedPerson) {
                                            hasConsecutiveConflict = true;
                                        }
                                    }
                                ...
                                if (hasConsecutiveConflict && assignedPerson) {
                                    ...
                                    if (targetDayOfWeek !== null) {
                                        ...
                                        const checkSwapCandidatePreview = (candidateDayKey, candidateDayIndex) => {
                                            ...
                                            // Check if swap person has conflict on current day (where they would be swapped to)
                                            // Calculate day after current day
                                            const currentDayAfter = new Date(date);
                                            currentDayAfter.setDate(currentDayAfter.getDate() + 1);
                                            const currentDayAfterKey = formatDateKey(currentDayAfter);
                                            const currentAfterType = getDayType(currentDayAfter);
                                            
                                            let swapPersonHasConflict = false;
                                            
                                            // Check if swap person has conflict on day after current day
                                            if (currentAfterType === 'semi-normal-day') {
                                                const swapPersonAfter = simulatedSemiAssignments[currentDayAfterKey]?.[groupNum];
                                                if (swapPersonAfter === swapPerson) {
                                                    swapPersonHasConflict = true;
                                                }
                                            }
                                            
                                            // Also check day before current day
                                            const currentDayBefore = new Date(date);
                                            currentDayBefore.setDate(currentDayBefore.getDate() - 1);
                                            const currentDayBeforeKey = formatDateKey(currentDayBefore);
                                            const currentBeforeType = getDayType(currentDayBefore);
                                            
                                            if (!swapPersonHasConflict && currentBeforeType === 'semi-normal-day') {
                                                const swapPersonBefore = simulatedSemiAssignments[currentDayBeforeKey]?.[groupNum];
                                                if (swapPersonBefore === swapPerson) {
                                                    swapPersonHasConflict = true;
                                                }
                                            }
                                            
                                            // Also check if swap person has conflict on swap day (where they currently would be)
                                            if (!swapPersonHasConflict) {
                                                const swapDate = new Date(candidateDayKey + 'T00:00:00');
                                                const swapDayAfter = new Date(swapDate);
                                                swapDayAfter.setDate(swapDayAfter.getDate() + 1);
                                                const swapDayAfterKey = formatDateKey(swapDayAfter);
                                                const swapAfterType = getDayType(swapDayAfter);
                                                
                                                if (swapAfterType === 'semi-normal-day') {
                                                    const swapPersonOnSwapDayAfter = simulatedSemiAssignments[swapDayAfterKey]?.[groupNum];
                                                    if (swapPersonOnSwapDayAfter === swapPerson) {
                                                        swapPersonHasConflict = true;
                                                    }
                                                }
                                            }
                                            
                                            // Also check if skipped person has conflict on swap day (where they would go)
                                            let skippedPersonHasConflict = false;
                                            if (!swapPersonHasConflict) {
                                                const swapDate = new Date(candidateDayKey + 'T00:00:00');
                                                const swapDayAfter = new Date(swapDate);
                                                swapDayAfter.setDate(swapDayAfter.getDate() + 1);
                                                ...
                                                if (swapAfterType === 'semi-normal-day') {
                                                    const skippedPersonOnSwapDayAfter = simulatedSemiAssignments[swapDayAfterKey]?.[groupNum];
                                                    if (skippedPersonOnSwapDayAfter === skippedPerson) {
                                                        skippedPersonHasConflict = true;
                                                    }
                                                }
                                            }
                                    ...
                                    if (swapDayKey !== null && swapDayIndex !== null) {
                                        ...
                                        // Check if swap person doesn't have conflict on current day AND swap day (restore full checking)
                                        if (swapPerson && !isPersonMissingOnDate(swapPerson, groupNum, date)) {
                                            let swapPersonHasConflict = false;
                                            
                                            // FIRST: Check if swap person has conflict on day BEFORE current day (where they would be swapped to)
                                            const dayBefore = new Date(date);
                                            dayBefore.setDate(dayBefore.getDate() - 1);
                                            const dayBeforeKey = formatDateKey(dayBefore);
                                            const beforeType = getDayType(dayBefore);
                                            
                                            if (beforeType === 'semi-normal-day') {
                                                const swapPersonBefore = simulatedSemiAssignments[dayBeforeKey]?.[groupNum];
                                                if (swapPersonBefore === swapPerson) {
                                                    swapPersonHasConflict = true;
                                                }
                                            }
                                            
                                            // SECOND: Check if swap person has conflict on current day (where they would be swapped to) - check day AFTER
                                            if (!swapPersonHasConflict) {
                                                const dayAfter = new Date(date);
                                                dayAfter.setDate(dayAfter.getDate() + 1);
                                                const dayAfterKey = formatDateKey(dayAfter);
                                                ...
                                                const swapPersonAfter = simulatedSemiAssignments[dayAfterKey]?.[groupNum];
                                                if (swapPersonAfter === swapPerson) {
                                                    swapPersonHasConflict = true;
                                                    }
                                                }
                                            ...
                                            // THIRD: Check if swap person has conflict on swap day (where they currently would be) - check day AFTER swap day
                                            if (!swapPersonHasConflict) {
                                            ...
                                            // FOURTH: Check if skipped person has conflict on swap day (where they would go) - check day AFTER swap day
                                            let skippedPersonHasConflict = false;
                                            if (!swapPersonHasConflict) {
                                                const swapDate = new Date(swapDayKey + 'T00:00:00');
                                                const swapDayAfter = new Date(swapDate);
                                            ...
                                            if (!swapPersonHasConflict && !skippedPersonHasConflict) {
                                                ...
                                                globalNormalRotationPosition[groupNum] = originalRotationPosition + 1;
                                        ...
                                        // No swap day found, still assign original person (they'll have conflict but it's unavoidable)
                                        globalNormalRotationPosition[groupNum] = rotationPosition + 1;
                                    }
                                } else {
                                    // No conflict, assign normally and advance rotation
        ...
        // Simplified assignment function for each day type (used by processCascadingSwaps)
        function assignDutiesForDayType(dayTypeCategory, groupNum, groupData, dayTypeLists) {
            const days = dayTypeLists[dayTypeCategory];
            if (days.length === 0) return;
            ...
            let groupPeople;
                    ...
                    // Track current rotation position globally (continues across months)
                    // For weekends: tracks position per group globally to continue from previous month
                    // For normal days: tracks position per group globally to continue from previous month
                    ...
                    // Track which people have been assigned to which days (to prevent duplicate assignments after swaps)
                    const assignedPeople = {}; // monthKey -> { groupNum -> Set of person names }
                    ...
                    days.forEach((dayKey, dayIndex) => {
                        ...
                        // Initialize assigned people set for this month if needed (for normal days)
                        if (dayTypeCategory === 'normal' && !assignedPeople[monthKey]) {
                            assignedPeople[monthKey] = {};
                        }
                        if (dayTypeCategory === 'normal' && !assignedPeople[monthKey][groupNum]) {
                        ...
                        if (dayTypeCategory === 'weekend') {
                            ...
                            rotationPosition = globalWeekendRotationPosition[groupNum] % rotationDays;
                            ...
                            // For special, semi: use rotation count from February 2026 directly
                            const daysSinceStart = getRotationPosition(dayDate, dayTypeCategory, groupNum);
                        ...
                        if (dayTypeCategory === 'normal' && expectedPerson && assignedPeople[monthKey] && assignedPeople[monthKey][groupNum] && assignedPeople[monthKey][groupNum].has(expectedPerson)) {
                            // DEBUG: Log that person was already assigned
                            console.log(`[ASSIGNMENT DEBUG] Person ${expectedPerson} was already assigned this month (likely swapped). Day: ${dayKey} (${dayDate.toLocaleDateString('el-GR')}), Rotation Position: ${rotationPosition}`);
                            console.log(`[ASSIGNMENT DEBUG] assignedPeople[${monthKey}][${groupNum}] contains:`, Array.from(assignedPeople[monthKey][groupNum]));
                            ...
                            for (let offset = 1; offset < rotationDays && !foundReplacement; offset++) {
                                ...
                                // Check if candidate was already assigned this month
                                if (assignedPeople[monthKey][groupNum].has(candidate)) {
                                    continue;
                                }
                            ...
                            // Check for conflicts based on day type
                                ...
                                if (hasSpecialHolidayDutyInMonth(expectedPerson, groupNum, month, year) || 
                                    ...
                                    if (hasConsecutiveDuty(dayKey, expectedPerson, groupNum)) {
                                        ...
                                        console.log(`[CONFLICT DEBUG] Person ${expectedPerson} has conflict on ${dayKey} (${dayDate.toLocaleDateString('el-GR')}), Day of Week: ${dayDate.getDay()} (${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][dayDate.getDay()]})`);
                                    }
                                }
                                
                            // If expected person has NO conflicts, assign them
                                // IMPORTANT: Re-check conflict using assignedPeople map to catch same-loop assignments
                                // This ensures we catch conflicts with assignments made earlier in the same loop iteration
                                const finalConflictCheck = hasConsecutiveDuty(dayKey, expectedPerson, groupNum, assignedPeople);
                                
                                if (!hasConflict && !finalConflictCheck) {
                                assignedPerson = expectedPerson;
                                // Track that this person is assigned (for normal days)
                                if (dayTypeCategory === 'normal' && assignedPeople[monthKey] && assignedPeople[monthKey][groupNum]) {
                                    assignedPeople[monthKey][groupNum].add(expectedPerson);
                                    // DEBUG: Log normal assignment
                                    console.log(`[ASSIGNMENT DEBUG] Assigned ${expectedPerson} to ${dayKey} (${dayDate.toLocaleDateString('el-GR')}), Rotation Position: ${rotationPosition}`);
                                    console.log(`[ASSIGNMENT DEBUG] assignedPeople[${monthKey}][${groupNum}] now contains:`, Array.from(assignedPeople[monthKey][groupNum]));
                                }
                                // Advance rotation position for next iteration
                                if (dayTypeCategory === 'weekend') {
                                    globalWeekendRotationPosition[groupNum] = rotationPosition + 1;
                                ...
                                if (finalConflictCheck && !hasConflict) {
                                    console.log(`[CONFLICT DETECTED] Person ${expectedPerson} has conflict on ${dayKey} due to same-loop assignment (detected in final check)`);
                                }
                                ...
                                if (dayTypeCategory === 'weekend') {
                                    ...
                                    storeAssignmentReason(dayKey, groupNum, skippedPerson, 'skip', reason);
                                        // Person will be skipped, so assignedPerson remains null and replacement will be found
                                    } else {
                                        ...
                                        // Store that person was assigned despite conflict
                                        storeAssignmentReason(dayKey, groupNum, assignedPerson, 'conflict', reason);
                                    }
                                }
                                ...
                                if (dayTypeCategory === 'normal' && skippedPerson && !swappedPeople.has(skippedPerson)) {
                                    ...
                                    if (targetDayOfWeek !== null) {
                                        // Helper function to check if a swap candidate is valid (no conflicts)
                                        const checkSwapCandidate = (candidateDayKey, candidateDayIndex) => {
                                            if (!candidateDayKey) return false;
                                            
                                            const swapRotationPosition = candidateDayIndex % rotationDays;
                                            const swapPerson = groupPeople[swapRotationPosition];
                                            
                                            if (!swapPerson || isPersonMissingOnDate(swapPerson, groupNum, dayDate)) {
                                                return false;
                                            }
                                            
                                            // Check if swap person was already assigned this month (due to previous swap)
                                            if (assignedPeople[monthKey] && assignedPeople[monthKey][groupNum] && assignedPeople[monthKey][groupNum].has(swapPerson)) {
                                                return false;
                                            }
                                            
                                            // Use enhanced conflict detection for swap person on current day (where they would be swapped to)
                                            let swapPersonHasConflict = hasConsecutiveDuty(dayKey, swapPerson, groupNum);
                                            
                                            // Also check if swap person has conflict on swap day (where they currently would be)
                                            if (!swapPersonHasConflict) {
                                                const swapDate = new Date(candidateDayKey + 'T00:00:00');
                                                if (!isPersonMissingOnDate(swapPerson, groupNum, swapDate)) {
                                                    swapPersonHasConflict = hasConsecutiveDuty(candidateDayKey, swapPerson, groupNum);
                                                }
                                            }
                                            
                                            // Check if skipped person has conflict on swap day (where they would go)
                                            let skippedPersonHasConflict = false;
                                            if (!swapPersonHasConflict && skippedPerson) {
                                                const swapDate = new Date(candidateDayKey + 'T00:00:00');
                                                if (!isPersonMissingOnDate(skippedPerson, groupNum, swapDate)) {
                                                    skippedPersonHasConflict = hasConsecutiveDuty(candidateDayKey, skippedPerson, groupNum);
                                                }
                                            }
                                        ...
                                        for (let i = dayIndex + 1; i < days.length; i++) {
                                            ...
                                            // Must be a normal day and in same week
                                                if (checkDayType === 'normal-day' && checkDayOfWeek === alternativeDayOfWeek && isSameWeek(dayDate, checkDate)) {
                                                // Check conflicts BEFORE accepting
                                                if (checkSwapCandidate(checkDayKey, i)) {
                                                        // Ask for permission before same-week swap
                                                        const swapRotationPosition = i % rotationDays;
                                                        const swapPerson = groupPeople[swapRotationPosition];
                                                        const currentDateStr = dayDate.toLocaleDateString('el-GR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                                                    ...
                                                    swapDayIndex = i;
                                                        ...
                                                        // If user rejected, continue to Step 2
                                                    }
                                                    // If has conflicts, continue to Step 2
                                                }
                                            }
                                        ...
                                        if (swapDayKey === null && swapDayIndex === null) {
                                            for (let i = dayIndex + 1; i < days.length; i++) {
                                                ...
                                                // Must be a normal day, same day of week, and in same month
                                                if (checkDayType === 'normal-day' && checkDayOfWeek === targetDayOfWeek && isSameMonth(dayDate, checkDate)) {
                                                    // Check conflicts BEFORE accepting
                                                    if (checkSwapCandidate(checkDayKey, i)) {
                                                        // Ask for permission before same-month swap
                                                        const swapRotationPosition = i % rotationDays;
                                                        const swapPerson = groupPeople[swapRotationPosition];
                                                        const currentDateStr = dayDate.toLocaleDateString('el-GR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                                                        const swapDateStr = checkDate.toLocaleDateString('el-GR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                                                        ...
                                                        // If user rejected, continue to Step 3
                                                    }
                                                    // If has conflicts, continue to Step 3
                                                }
                                            }
                                        ...
                                        if (!swapFound && swapDayKey === null && swapDayIndex === null) {
                                            // Collect all week-after-next candidates (both targetDayOfWeek and alternativeDayOfWeek)
                                            for (let i = dayIndex + 1; i < days.length; i++) {
                                                const checkDayKey = days[i];
                                                const checkDate = new Date(checkDayKey + 'T00:00:00');
                                                const checkDayOfWeek = checkDate.getDay();
                                                ...
                                                // Must be a normal day and in week AFTER next (not same week, not next week)
                                                if (checkDayType === 'normal-day' && !isSameWeek(dayDate, checkDate) && isWeekAfterNext(dayDate, checkDate)) {
                                                    // Check if it's either targetDayOfWeek or alternativeDayOfWeek
                                                    if (checkDayOfWeek === targetDayOfWeek || (alternativeDayOfWeek !== null && checkDayOfWeek === alternativeDayOfWeek)) {
                                                        // Check conflicts BEFORE adding to candidates
                                                        if (checkSwapCandidate(checkDayKey, i)) {
                                                            weekAfterNextCandidates.push({
                                                                dayKey: checkDayKey,
                                            ...
                                            // If found week-after-next candidates, ask for permission for the EARLIEST one
                                            if (weekAfterNextCandidates.length > 0) {
                                                const earliestCandidate = weekAfterNextCandidates[0];
                                                const swapDate = earliestCandidate.date;
                                                const swapRotationPosition = earliestCandidate.dayIndex % rotationDays;
                                                const swapPerson = groupPeople[swapRotationPosition];
                                    ...
                                    // If found swap day, find which person would be assigned to that day
                                    // NOTE: Conflict checks were already done in checkSwapCandidate, so we can proceed directly
                                    if (swapDayKey !== null && swapDayIndex !== null) {
                                        // Calculate rotation position for swap day
                                        const swapRotationPosition = swapDayIndex % rotationDays;
                                        const swapPerson = groupPeople[swapRotationPosition];
                                        ...
                                        // Verify swap person exists and is not missing (basic checks only)
                                        // Conflict checks were already done in checkSwapCandidate function
                                        if (swapPerson && !isPersonMissingOnDate(swapPerson, groupNum, dayDate) && 
                                            (!assignedPeople[monthKey] || !assignedPeople[monthKey][groupNum] || !assignedPeople[monthKey][groupNum].has(swapPerson))) {
                                            // Ask for permission before performing swap (if not already asked)
                                                const swapDate = new Date(swapDayKey + 'T00:00:00');
                                            ...
                                            if (!isSameMonthSwap && !isSameWeekSwap) {
                                                // For week-after-next, permission was already asked, so we proceed
                                                userApproved = true;
                                            }
                                            ...
                                            if (userApproved) {
                                                ...
                                                // DIRECTLY assign skipped person to swap day (don't wait for rotation)
                                                // This ensures the swap happens immediately
                                                assignPersonToDay(swapDayKey, skippedPerson, groupNum);
                                                
                                                // Track that skipped person has been assigned in BOTH months (current day's month AND swap day's month)
                                                // This prevents them from being assigned again on the next Monday or any other day in either month
                                                if (dayTypeCategory === 'normal') {
                                                    // Add to current day's month
                                                    if (!assignedPeople[monthKey]) assignedPeople[monthKey] = {};
                                                    if (!assignedPeople[monthKey][groupNum]) assignedPeople[monthKey][groupNum] = new Set();
                                                    ...
                                                    if (!assignedPeople[swapMonthKey]) assignedPeople[swapMonthKey] = {};
                                                    if (!assignedPeople[swapMonthKey][groupNum]) assignedPeople[swapMonthKey][groupNum] = new Set();
                                                    assignedPeople[swapMonthKey][groupNum].add(skippedPerson);
                                                }
                                                
                                                // Track that swap person has been assigned (to prevent duplicate assignment later)
                                                if (dayTypeCategory === 'normal') {
                                                    if (!assignedPeople[monthKey]) assignedPeople[monthKey] = {};
                                                    if (!assignedPeople[monthKey][groupNum]) assignedPeople[monthKey][groupNum] = new Set();
                                                    assignedPeople[monthKey][groupNum].add(swapPerson);
                                                }
                                                
                                                // Advance rotation position to original expected person's position + 1
                                                // This ensures rotation continues normally: if Person 5 swaps with Person 2,
                                                // rotation continues from Person 3 (position 2), not from Person 6 (position 5)
                                                if (dayTypeCategory === 'normal') {
                                                    globalNormalRotationPosition[groupNum] = originalRotationPosition + 1;
                                                ...
                                                swappedDays[swapDayKey][groupNum] = true;
                                                
                                                // Mark both people as swapped to prevent swapping them again on subsequent days
                                                swappedPeople.add(skippedPerson);
                                                swappedPeople.add(swapPerson);
                                            } else {
                                                // User rejected swap - clear swap and try to find replacement
                                                swapDayKey = null;
                                                swapDayIndex = null;
                                            }
                                        }
                                    }
                                } else if (dayTypeCategory === 'normal' && skippedPerson && swappedPeople.has(skippedPerson)) {
                                    // Person was already swapped - don't attempt another swap, just find a replacement
                                    // assignedPerson remains null, so replacement logic below will handle it
                                }
                                
                                } // Close else block for conflict check
                                
                                // If no swap found for normal days, or for other day types, find replacement from rotation
                                if (!assignedPerson) {
                                    ...
                                    for (let offset = 1; offset < rotationDays && !foundReplacement; offset++) {
                                        ...
                                        // Use enhanced hasConsecutiveDuty for comprehensive conflict checking
                                        // Pass assignedPeople map to check in-progress assignments from same loop
                                        // This checks all combinations: normal↔semi, normal↔weekend, normal↔special, semi↔weekend, semi↔special
                                        candidateHasConflict = hasConsecutiveDuty(dayKey, candidate, groupNum, assignedPeople);
                                        
                                        // Additional checks for weekend day type
                                        if (dayTypeCategory === 'weekend') {
                                        ...
                                        if (!candidateHasConflict) {
                                            ...
                                            if (dayTypeCategory === 'weekend') {
                                                ...
                                                globalNormalRotationPosition[groupNum] = nextIndex + 1;
                                            }
                                            break;
                                        }
                                    }
                        ...
                        if (!assignedPerson) {
                            for (let i = 0; i < groupPeople.length; i++) {
                                ...
                                // Additional checks for weekend day type
                                if (dayTypeCategory === 'weekend') {
                                    // Check for special holiday in same month
                                    if (hasSpecialHolidayDutyInMonth(candidate, groupNum, month, year)) {
                                        candidateHasConflict = true;
                                    }
                                }
                                ...
                                if (!candidateHasConflict) {
                                    assignedPerson = candidate;
                                    console.log(`[EMERGENCY FALLBACK] Assigned ${candidate} to ${dayKey} (no conflicts found)`);
                                    break;
                                }
                            }
                        }
                        ...
                        if (skippedPerson && skippedPerson !== assignedPerson && dayTypeCategory !== 'weekend') {
                            ...
                            if (nextAvailableDay) {
                                ...
                                // If found swap day, find which person would be assigned to that day and swap
                                if (swapDayKey && swapDayIndex !== null) {
                                    // DEBUG: Log swap day found
                                    let swapDate = new Date(swapDayKey + 'T00:00:00');
                                    console.log(`[SWAP ATTEMPT DEBUG] Found swap day: ${swapDayKey} (${swapDate.toLocaleDateString('el-GR')}), Day of Week: ${swapDate.getDay()} (${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][swapDate.getDay()]})`);
                                    
                                    // Calculate rotation position for swap day
                                    let swapRotationPosition = swapDayIndex % rotationDays;
                                    let swapPerson = groupPeople[swapRotationPosition];
                                    console.log(`[SWAP ATTEMPT DEBUG] Swap person at rotation position ${swapRotationPosition}: ${swapPerson}`);
                                    ...
                                    let swapYear = swapDate.getFullYear();
                                    let swapMonthKey = `${swapYear}-${swapMonth}`;
                                    let swapPersonAlreadyAssigned = assignedPeople[swapMonthKey] && assignedPeople[swapMonthKey][groupNum] && assignedPeople[swapMonthKey][groupNum].has(swapPerson);
                                    
                                    console.log(`[SWAP ATTEMPT DEBUG] Swap day already assigned: ${swapDayAlreadyAssigned}, Swap person: ${swapPerson}, Swap person missing: ${swapPerson ? isPersonMissingOnDate(swapPerson, groupNum, swapDate) : 'N/A'}, Swap person already assigned this month: ${swapPersonAlreadyAssigned}`);
                                    
                                    // If swap person was already assigned, this swap day is not valid - need to find another
                                    if (swapPersonAlreadyAssigned) {
                                        console.log(`[SWAP ATTEMPT DEBUG] Cannot use swap day ${swapDayKey} - swap person ${swapPerson} was already assigned this month. Need to find another swap day.`);
                                        swapDayKey = null;
                                        swapDayIndex = null;
                                        
                                        // Try to find next available swap day (continue searching)
                                        // Look for next alternative day or next same day
                                        if (alternativeDayOfWeek !== null) {
                                            for (let i = dayIndex + 1; i < days.length; i++) {
                                                const checkDayKey = days[i];
                                                const checkDate = new Date(checkDayKey + 'T00:00:00');
                                                ...
                                                if (checkDayType === 'normal-day' && checkDayOfWeek === alternativeDayOfWeek) {
                                                    const checkSwapRotationPosition = i % rotationDays;
                                                    const checkSwapPerson = groupPeople[checkSwapRotationPosition];
                                                    const checkSwapMonth = checkDate.getMonth();
                                                    const checkSwapYear = checkDate.getFullYear();
                                                    const checkSwapMonthKey = `${checkSwapYear}-${checkSwapMonth}`;
                                                    const checkSwapPersonAssigned = assignedPeople[checkSwapMonthKey] && assignedPeople[checkSwapMonthKey][groupNum] && assignedPeople[checkSwapMonthKey][groupNum].has(checkSwapPerson);
                                                    
                                                    if (!checkSwapPersonAssigned) {
                                                        swapDayKey = checkDayKey;
                                                        swapDayIndex = i;
                                                        console.log(`[SWAP ATTEMPT DEBUG] Found alternative swap day: ${swapDayKey} (${checkDate.toLocaleDateString('el-GR')}) with available person ${checkSwapPerson}`);
                                                        break;
                                                    }
                                                }
                                            }
                                        ...
                                        // If still no swap day, try next same day
                                        if (!swapDayKey && targetDayOfWeek !== null) {
                                            for (let i = dayIndex + 1; i < days.length; i++) {
                                                const checkDayKey = days[i];
                                                const checkDate = new Date(checkDayKey + 'T00:00:00');
                                                ...
                                                if (checkDayType === 'normal-day' && checkDayOfWeek === targetDayOfWeek) {
                                                    ...
                                                    if (!checkSwapPersonAssigned) {
                                                        swapDayKey = checkDayKey;
                                                        swapDayIndex = i;
                                                        console.log(`[SWAP ATTEMPT DEBUG] Found next same day swap: ${swapDayKey} (${checkDate.toLocaleDateString('el-GR')}) with available person ${checkSwapPerson}`);
                                                        break;
                                                    }
                                                }
                                            }
                                        ...
                                        // Recalculate swap person if we found a new swap day
                                        if (swapDayKey && swapDayIndex !== null) {
                                            const newSwapRotationPosition = swapDayIndex % rotationDays;
                                            const newSwapPerson = groupPeople[newSwapRotationPosition];
                                            const newSwapDate = new Date(swapDayKey + 'T00:00:00');
                                            
                                            // Update swap person and date (swapDate is let, so can be reassigned)
                                            swapPerson = newSwapPerson;
                                            swapDate = newSwapDate;
                                            swapMonth = newSwapDate.getMonth();
                                            swapYear = newSwapDate.getFullYear();
                                            swapMonthKey = `${swapYear}-${swapMonth}`;
                                            swapPersonAlreadyAssigned = assignedPeople[swapMonthKey] && assignedPeople[swapMonthKey][groupNum] && assignedPeople[swapMonthKey][groupNum].has(newSwapPerson);
                                            
                                            console.log(`[SWAP ATTEMPT DEBUG] Recalculated swap person: ${swapPerson} for swap day ${swapDayKey} (${swapDate.toLocaleDateString('el-GR')}), Already assigned: ${swapPersonAlreadyAssigned}`);
                                        }
                                    }
                                    
                                    if (swapDayKey && swapDayIndex !== null && !swapDayAlreadyAssigned && swapPerson && !isPersonMissingOnDate(swapPerson, groupNum, swapDate) && !swapPersonAlreadyAssigned) {
                                        // Check if swap person has conflict on swap day (where they currently would be)
                                        const swapDayAfter = new Date(swapDate);
                                        swapDayAfter.setDate(swapDayAfter.getDate() + 1);
                                        const swapDayAfterKey = formatDateKey(swapDayAfter);
                                        const swapAfterType = getDayType(swapDayAfter);
                                        ...
                                        let swapPersonHasConflict = false;
                                        if (swapAfterType === 'semi-normal-day' && hasDutyOnDay(swapDayAfterKey, swapPerson, groupNum)) {
                                            swapPersonHasConflict = true;
                                        }
                                        
                                        // Also check if swap person has conflict on current day (where they would be swapped to)
                                        if (!swapPersonHasConflict) {
                                            const dayAfter = new Date(dayDate);
                                            dayAfter.setDate(dayAfter.getDate() + 1);
                                            const dayAfterKey = formatDateKey(dayAfter);
                                            ...
                                            if (afterType === 'semi-normal-day' && hasDutyOnDay(dayAfterKey, swapPerson, groupNum)) {
                                                swapPersonHasConflict = true;
                                            }
                                        }
                                        ...
                                        if (!swapPersonHasConflict && skippedPerson) {
                                            if (!isPersonMissingOnDate(skippedPerson, groupNum, swapDate)) {
                                                if (swapAfterType === 'semi-normal-day' && hasDutyOnDay(swapDayAfterKey, skippedPerson, groupNum)) {
                                                    skippedPersonHasConflict = true;
                                                }
                                            }
                                        }
                                        ...
                                        if (!swapPersonHasConflict && !skippedPersonHasConflict) {
                                            // Valid swap: assign skipped person to swap day
                                            assignPersonToDay(swapDayKey, skippedPerson, groupNum);
                                            
                                            // CRITICAL: Track that skipped person was assigned to swap day
                                            // This prevents them from being assigned again on their original rotation position (next Monday)
                                            // swapDate is already defined above in the outer scope
                                            const finalSwapDate = new Date(swapDayKey + 'T00:00:00');
                                            const swapMonth = finalSwapDate.getMonth();
                                            const swapYear = finalSwapDate.getFullYear();
                                            const swapMonthKey = `${swapYear}-${swapMonth}`;
                                            
                                            if (!assignedPeople[swapMonthKey]) {
                                                assignedPeople[swapMonthKey] = {};
                                            }
                                            if (!assignedPeople[swapMonthKey][groupNum]) {
                                                assignedPeople[swapMonthKey][groupNum] = new Set();
                                            }
                                            assignedPeople[swapMonthKey][groupNum].add(skippedPerson);
                                            
                                            // DEBUG: Log the swap
                                            console.log(`[SWAP DEBUG] Person ${skippedPerson} swapped from ${dayKey} (${dayDate.toLocaleDateString('el-GR')}) to ${swapDayKey} (${swapDate.toLocaleDateString('el-GR')})`);
                                            console.log(`[SWAP DEBUG] Added ${skippedPerson} to assignedPeople[${swapMonthKey}][${groupNum}]`);
                                            console.log(`[SWAP DEBUG] assignedPeople[${swapMonthKey}][${groupNum}] now contains:`, Array.from(assignedPeople[swapMonthKey][groupNum]));
