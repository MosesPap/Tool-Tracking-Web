        ...
        async function applyAlternateReplacementForDate(groupNum, personName, dateKey) {
            try {
                ...
                const testMap = { ...groupMap, [groupNum]: replacement };
                const testParts = [];
                for (let g = 1; g <= 4; g++) {
                    const pn = testMap[g];
        ...
        function getDutyAssistantKnowledgeBase() {
            return [
                {
                    id: 'calculate',
                    keywords: ['υπολογισμ', 'calculate', 'βημα', 'step', 'εκτελεσ', 'υπηρεσι', 'ημερομηνια', 'start', 'end', 'μηνας κλειδωμενος'],
                    answer:
...
Συμβουλή: Αν ο μήνας είναι κλειδωμένος (toggle «Κλείδωμα μήνα»), πρώτα ξεκλειδώστε τον.`
                ...
                {
                    id: 'swap-logic',
                    keywords: ['swap', 'αλλαγ', 'συγκρουσ', 'δευτερα', 'τεταρτη', 'τριτη', 'πεμπτη', 'consecutive', 'διπλη', 'συνεχομεν', 'αντικατασταση'],
                    answer:
...
Σε κάθε περίπτωση, το σύστημα ελέγχει ώστε να μη δημιουργούνται νέες συγκρούσεις και να μη γίνεται swap με απόντες/απενεργοποιημένους.`
                ...
                {
                    id: 'missing-disabled',
                    keywords: ['απουσια', 'απων', 'απενεργ', 'disabled', 'missing', 'περιοδο', 'λογο', 'reason', 'buffer', 'day before', 'day after'],
                    answer:
...
Το σύστημα αποφεύγει αυτές τις αναθέσεις στον υπολογισμό και τις λαμβάνει υπόψη σε swaps/αντικαταστάσεις.`
                ...
                {
                    id: 'person-actions',
                    keywords: ['ενεργειες ατομου', 'person actions', 'μεταφορα', 'transfer', 'θεση', 'καταταξη', 'rank', 'manual'],
                    answer:
`Από τις «Ενέργειες Ατόμου» μπορείτε να:
- αλλάξετε σειρά/θέση του ατόμου στη λίστα,
- μεταφέρετε άτομο σε άλλη ομάδα,
- ορίσετε απουσία/απενεργοποίηση.

Αν αλλάξει ομάδα ή θέση, το σύστημα προσαρμόζει τη συνέχεια της περιστροφής στον επόμενο υπολογισμό.`
                },
                {
                    id: 'calendar-modes',
                    keywords: ['ημερολογιο', 'μονο αποντες', 'hover', 'popup', 'cell', 'κελι', 'search', 'αναζητηση', 'highlight'],
                    answer:
`Στο ημερολόγιο:
...
- Μπορείτε να αλλάξετε το ύψος κελιού από «Ύψος κελιών».`
                ...
                {
                    id: 'print',
                    keywords: ['εκτυπωση', 'σειρα', 'ιεραρχια', 'print', 'rank', 'pdf', 'χαρτι'],
                    answer:
...
Στις σειρές, τα απενεργοποιημένα άτομα σημειώνονται με διακριτή μορφοποίηση και οι απουσίες εμφανίζονται με τρέχον/μελλοντικό εύρος ημερομηνιών.`
                ...
                {
                    id: 'excel',
                    keywords: ['excel', 'εξαγωγη', 'αρχειο', 'month picker', 'δημιουργια excel'],
                    answer:
`Για εξαγωγή Excel:
1) Πατήστε «Δημιουργία Excel».
2) Επιλέξτε μήνα.
3) Επιβεβαιώστε δημιουργία αρχείων.

Αν δεν δείτε σωστά δεδομένα, κάντε πρώτα υπολογισμό και αποθήκευση για την περίοδο που σας ενδιαφέρει.`
                },
                {
                    id: 'settings',
                    keywords: ['ρυθμισ', 'settings', 'firebase', 'καθαρισμ', 'toggle', 'switch', 'logic'],
                    answer:
...
Οι επιλογές εξαιρέσεων αποθηκεύονται μόνιμα μέχρι να τις αλλάξετε.`
                ...
                {
                    id: 'lock-month',
                    keywords: ['κλειδωμα', 'μηνας', 'lock month', 'ξεκλειδωμα', 'recalculate'],
                    answer:
`Το «Κλείδωμα μήνα» εμποδίζει νέο υπολογισμό για τον μήνα που βλέπετε.
- Κλειδωμένος μήνας: δεν ξεκινά υπολογισμός.
- Ξεκλείδωτος μήνας: επιτρέπεται νέος υπολογισμός.

Χρησιμοποιήστε το όταν θέλετε να προστατέψετε οριστικοποιημένο πρόγραμμα.`
                },
                {
                    id: 'ranking',
                    keywords: ['ιεραρχια', 'ranking', 'rankings', 'σειρα ιεραρχιας', 'drag', 'drop'],
                    answer:
`Στη «Διαχείριση Ιεραρχίας» μπορείτε να αλλάξετε τη σειρά με drag & drop ή αριθμητικά.
Η ιεραρχία είναι ανεξάρτητη από τις λίστες ομάδων και χρησιμοποιείται όπου χρειάζεται ιεραρχική απεικόνιση/κανόνας.`
                },
                {
                    id: 'troubleshooting',
                    keywords: ['γιατι', 'δεν', 'why', 'problem', 'σφαλμα', 'error', 'δεν εγινε', 'δεν σωζει', 'save'],
                    answer:
`Αν κάτι δεν βγαίνει όπως περιμένετε, κάντε αυτόν τον έλεγχο:
1) Είναι σωστός ο μήνας και ξεκλείδωτος;
2) Υπάρχουν απουσίες/απενεργοποιήσεις που μπλοκάρουν ανάθεση;
3) Έγινε «Αποθήκευση» στο αντίστοιχο modal;
4) Για swaps: ελέγξτε αν υπάρχουν έγκυρες εναλλακτικές χωρίς νέες συγκρούσεις.

Αν θέλετε, περιγράψτε μου συγκεκριμένη ημερομηνία/ομάδα για πιο στοχευμένη βοήθεια.`
                }
            ];
        }
        function getDutyAssistantReply(questionRaw) {
            ...
            for (const item of kb) {
                ...
                for (const kw of item.keywords) {
                    ...
                    if (!nkw) continue;
                    if (q.includes(nkw)) score += Math.max(2, Math.floor(nkw.length / 3));
                    const kwTokens = nkw.split(' ').filter((t) => t.length >= 2);
                    let overlap = 0;
                    for (const t of kwTokens) {
                        if (qTokens.includes(t)) overlap++;
                    }
...
- «Τι κάνει το κλείδωμα μήνα;»`;
            ...
            if (second && second.score >= Math.max(3, best.score - 1)) {
