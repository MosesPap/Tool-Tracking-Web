        ...
        function updateExcludeFromDutiesActionButton(groupNum, personName) {
            try {
                const excluded =
                    typeof isPersonExcludedFromDuties === 'function' &&
        ...
        function openPersonActionsModal(groupNum, personName, index, listType) {
            ...
            updateExcludeFromDutiesActionButton(groupNum, personName);

            const modalEl = document.getElementById('personActionsModal');
        ...
        /**
         * Ενιαία αντικατάσταση επιλαχόντα (ίδια για «Ενέργειες ατόμου» και modal ημέρας).
         * @param {number} groupNum
         * @param {string} skippedPerson — αυτός που παραλείπεται (ήταν ανατεθειμένος)
         * @param {string} dateKey
         * @param {{ replacement?: string|null, skipFinalSave?: boolean, skipCalendarReload?: boolean }} [options]
         * @returns {Promise<boolean>} false αν αποτυχία ελέγχου
         */
        async function applyManualAlternateReplacementForDate(groupNum, skippedPerson, dateKey, options = {}) {
            const replacementExplicit = options.replacement != null && String(options.replacement).trim()
                ? String(options.replacement).trim()
                : null;
            const skipFinalSave = options.skipFinalSave === true;
            const skipCalendarReload = options.skipCalendarReload === true;
            ...
            if (!['normal', 'semi', 'weekend', 'special'].includes(typeCategory)) {
                alert('Η αντικατάσταση επιλαχόντα ισχύει μόνο για καθημερινές, ημιαργίες, αργίες και ειδικές αργίες.');
                return false;
            }
            ...
            const nSkip = norm(skippedPerson);
            const nAssigned = norm(assignedForGroup);
            if (!nSkip) {
                alert('Δεν ορίστηκε το άτομο που παραλείπεται.');
                return false;
            }
            if (nAssigned !== nSkip && (!replacementExplicit || nAssigned !== norm(replacementExplicit))) {
                alert('Το άτομο δεν είναι ανατεθειμένο σε υπηρεσία σε αυτή την ημερομηνία (ή η ανάθεση άλλαξε ήδη).');
                return false;
            }

            let replacement = replacementExplicit;
            if (!replacement) {
                let startIdx = list.findIndex(p => norm(p) === nSkip);
                ...
                if (typeof isPersonMissingOnDate === 'function' && isPersonMissingOnDate(replacement, groupNum, dateObj, typeCategory)) {
                    alert('Ο/η επιλεγμένος/η επιλαχόνος/α είναι απόντης/ουσα σε αυτή την ημερομηνία.');
                    return false;
                }
            }
            if (!replacement) {
            ...
            if (norm(replacement) === nSkip) {
                alert('Ο/η επιλαχόνος/α πρέπει να είναι διαφορετικός/ή από τον/την απόντα/ούσα.');
                return false;
            }

            if (
                !options.skipConsecutiveConflictCheck &&
                wouldManualDutySelectionCauseConsecutiveConflict(dateKey, replacement, groupNum, skippedPerson, 'replacement')
            ) {
            ...
            if (typeof clearAssignmentReasonForPersonOnDate === 'function') {
                clearAssignmentReasonForPersonOnDate(dateKey, groupNum, skippedPerson);
            }
        ...
        async function applyAlternateReplacementForDate(groupNum, personName, dateKey) {
            try {
                const ok = await applyManualAlternateReplacementForDate(groupNum, personName, dateKey, {});
                if (!ok) return;
            } catch (e) {
        ...
        function openMissingDisabledPeopleModal() {
            ...
            for (let groupNum = 1; groupNum <= 4; groupNum++) {
                ...
                allPeople.forEach((person) => {
                    ...
                    if (
                        disabledState.all ||
                        disabledState.special ||
                        disabledState.weekend ||
                        disabledState.semi ||
                ...
                groupSection.innerHTML =
                    ...
                    (excludedPeople.length + disabledPeople.length + missingPeople.length) +
                    ' άτομα</span></h5>';

                const peopleList = document.createElement('div');
                peopleList.className = 'list-group';

                excludedPeople.forEach((person) => {
                    ...
                    item.innerHTML =
                        ...
                        person +
                        '</strong>' +
                        '<span class="badge bg-dark ms-2">Εξαίρεση</span>' +
                        '<div class="text-muted small mt-1">' +
                        '<i class="fas fa-ban me-1"></i>Δεν συμμετέχει στον υπολογισμό υπηρεσιών / σειρά' +
                        '</div></div>' +
                        '<button class="btn btn-sm btn-outline-primary" onclick="editPersonStatus(' +
                        groupNum +
                        ", '" +
                        safeName +
                        "')\">" +
                        '<i class="fas fa-edit me-1"></i>Επεξεργασία</button>';
                ...
                missingPeople.forEach(({ person, periods }) => {
                    ...
                    const reasonDisplay =
                        uniqueReasons.length > 0 ? uniqueReasons.join(', ') : 'Απουσία';
            ...
            if (!hasAnyPeople) {
                container.innerHTML =
                    ...
                    '<strong>Καμία εγγραφή!</strong> Δεν υπάρχουν απενεργοποιημένα, εξαιρεμένα ή απουσιάζοντα άτομα.' +
                    '</div>';
            }

            const modalEl = document.getElementById('missingDisabledPeopleModal');
            bootstrap.Modal.getOrCreateInstance(modalEl).show();
        }
        function editPersonStatus(groupNum, personName) {
            const missingModal = bootstrap.Modal.getInstance(
                document.getElementById('missingDisabledPeopleModal')
            );
            if (missingModal) missingModal.hide();

            const resolveActionsContext = () => {
                currentPersonActionsGroup = groupNum;
                currentPersonActionsName = personName;
                let foundIndex = -1;
            ...
            if (isExcluded) {
                if (!resolveActionsContext()) {
                    alert(
                        'Το άτομο "' +
                            personName +
                            '" δεν βρέθηκε στις λίστες της ομάδας ' +
                            getGroupName(groupNum) +
                            '.'
                    );
                    return;
                ...
                if (year === 2026 && month === 9) {
                    const parseG = (raw, g) => {
                        ...
                        if (typeof raw === 'object' && !Array.isArray(raw)) {
                            return raw[g] || raw[String(g)] || null;
                        }
                        const s = String(raw);
                        const parts = s.split(',').map((p) => p.trim());
                        for (const part of parts) {
                            const m = part.match(/^(.+?)\s*\(Ομάδα\s*(\d+)\)\s*$/);
                            if (m && parseInt(m[2], 10) === g) return m[1].trim();
                        }
                        return null;
                    };
                    const tempW =
                        (typeof calculationSteps !== 'undefined' && calculationSteps.tempWeekendAssignments) ||
                        {};
                    const snap = {};
                    for (const dk of ['2026-10-24', '2026-10-25', '2026-10-28', '2026-10-31']) {
                        const saved = typeof getAssignmentForDate === 'function' ? getAssignmentForDate(dk) : null;
                        snap[dk] = {
                            dayType: getDayType(new Date(dk + 'T00:00:00')),
                            savedG3: parseG(saved, 3),
                            savedG1: parseG(saved, 1),
                            weekendStoreG3: parseG(weekendAssignments?.[dk], 3),
                            tempG3: tempW?.[dk]?.[3] || null,
                            tempG1: tempW?.[dk]?.[1] || null
                        };
                    }
                    const row = {
                        sessionId: '8e8ea0',
                        runId: 'alex-month-dupe',
                        hypothesisId: 'H-cal-saved',
                        ...
                        data: {
                            ...
                            alexSavedDates: Object.entries(snap)
                                ...
                                .map(([dk]) => dk),
                            alexTempDates: Object.entries(snap)
                                .filter(([, v]) => v.tempG3 && String(v.tempG3).includes('ΑΛΕΞΑΝΔΡΟΥ'))
                                .map(([dk]) => dk)
                        },
                        timestamp: Date.now()
                    };
        ...
        async function saveDayAssignments() {
            ...
            selects.forEach(select => {
                ...
                const prevPerson = typeof parseAssignedPersonForGroupFromAssignment === 'function'
                    ...
                    : '';
                const newVal = select.value.trim();
                if (normPerson(prevPerson) === normPerson(newVal)) return;
                if (!newVal) return;
                const modeEl = container.querySelector(`input[name="duty-change-mode-${group}"]:checked`);
                const mode = modeEl ? modeEl.value : 'replacement';
                ...
                if (prevPerson && typeof clearAssignmentReasonForPersonOnDate === 'function') {
                    clearAssignmentReasonForPersonOnDate(dayKey, group, prevPerson);
                }
                const reason = `Χειροκίνητη ανάθεση: ${newVal} ${getGreekDayAccusativeArticle(dateObj)} ${getGreekDayName(dateObj)} ${dateStr}.`;
                storeAssignmentReason(dayKey, group, newVal, 'skip', reason, prevPerson || null, null, { manualDayModal: true });
            });
